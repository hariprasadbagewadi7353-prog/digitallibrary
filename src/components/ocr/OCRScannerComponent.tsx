...
export const OCRScannerComponent: React.FC<OCRScannerComponentProps> = ({
  ...
  const startCamera = async () => {
    try {
      ...
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        throw new Error('Camera access is not supported in this browser environment. Please use file upload.');
      }
  ...
  // Sample Records for Instant Testing
  ...
  return (
    <div id="ocr-scanner-root" className="space-y-6">
      {/* Header & Controls */}
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 md:p-6 shadow-xs border border-slate-200 dark:border-slate-800 transition-colors">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">Quick Test:</span>
            <button
              onClick={() => loadSampleRecord('MEMBER')}
              className="px-3 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-semibold rounded-lg border border-slate-300 dark:border-slate-700 transition-colors cursor-pointer"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mt-5 pt-4 border-t border-slate-100 dark:border-slate-800">
            ...
            <div className="flex rounded-lg border border-slate-200 dark:border-slate-700 p-1 bg-slate-50 dark:bg-slate-800">
              <button
                ...
                className={`flex-1 py-1.5 text-xs font-semibold rounded-md transition-all cursor-pointer ${
                  docType === 'MEMBER' ? 'bg-white dark:bg-slate-700 text-amber-700 dark:text-amber-300 shadow-xs border border-slate-200 dark:border-slate-600' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
                }`}
              >
                Member Card
              </button>
              <button
                ...
                className={`flex-1 py-1.5 text-xs font-semibold rounded-md transition-all cursor-pointer ${
                  docType === 'BOOK' ? 'bg-white dark:bg-slate-700 text-amber-700 dark:text-amber-300 shadow-xs border border-slate-200 dark:border-slate-600' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
                }`}
              >
                Book Catalog
              </button>
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-xs font-medium text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-amber-500"
            >
          ...
          <div className="sm:col-span-2 flex items-end gap-2">
            <button
              ...
              className="flex-1 bg-amber-500 hover:bg-amber-400 disabled:bg-slate-200 dark:disabled:bg-slate-800 disabled:text-slate-400 text-slate-950 font-bold py-2.5 px-4 rounded-xl text-sm transition-all shadow-xs flex items-center justify-center gap-2 cursor-pointer"
            >
              {isProcessing ? (
                <>
            ...
            {imageSrc && (
              <button
                ...
                className="p-2.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/50 rounded-xl border border-slate-200 dark:border-slate-700 transition-colors cursor-pointer"
                title="Clear Image"
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Left Column: Image Capture & Preprocessing (6 cols) */}
        <div className="lg:col-span-6 space-y-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 shadow-xs border border-slate-200 dark:border-slate-800 transition-colors">
              ...
              <div className="flex rounded-lg border border-slate-200 dark:border-slate-700 p-0.5 bg-slate-100 dark:bg-slate-800">
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-md transition-all cursor-pointer ${
                    activeSource === 'upload' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-xs' : 'text-slate-600 dark:text-slate-400'
                  }`}
                >
                ...
                <button
                  ...
                  onClick={startCamera}
                  className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-md transition-all cursor-pointer ${
                    activeSource === 'camera' ? 'bg-white dark:bg-slate-700 text-amber-700 dark:text-amber-300 shadow-xs' : 'text-slate-600 dark:text-slate-400'
                  }`}
                >
            ...
            {activeSource === 'camera' && isCameraActive && (
              <div className="relative rounded-xl overflow-hidden bg-slate-950 border border-slate-800 aspect-video flex items-center justify-center">
                ...
                <button
                  ...
                  className="absolute bottom-4 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold px-6 py-2.5 rounded-full text-xs shadow-lg active:scale-95 transition-all flex items-center gap-2 cursor-pointer"
                >
            ...
            {cameraError && activeSource === 'camera' && !isCameraActive && (
              <div className="p-4 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded-xl text-amber-800 dark:text-amber-300 text-xs flex items-start gap-2">
                <AlertCircle className="w-4 h-4 shrink-0 text-amber-600 mt-0.5" />
                <div>
                  <p className="font-semibold">Camera Access Note:</p>
                  <p className="mt-0.5">{cameraError}</p>
                  <p className="mt-2 text-slate-600 dark:text-slate-400 font-medium">
                    You can simply upload a photo or use the "Quick Test" sample buttons above.
                  </p>
                </div>
              </div>
            )}
              ...
              <div>
                {!imageSrc ? (
                  <div
                    ...
                    className="border-2 border-dashed border-slate-300 dark:border-slate-700 hover:border-amber-500 rounded-xl p-8 text-center cursor-pointer bg-slate-50/50 dark:bg-slate-800/40 hover:bg-amber-50/20 dark:hover:bg-slate-800/80 transition-all group"
                  >
                    <input
                      ref={fileInputRef}
                      type="file"
                      ...
                      className="hidden"
                    />
                    <div className="w-12 h-12 rounded-full bg-amber-100 dark:bg-amber-950/60 text-amber-700 dark:text-amber-400 flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition-transform">
                  ...
                  <div className="space-y-3">
                    <div className="relative bg-slate-900 rounded-xl overflow-hidden border border-slate-200 dark:border-slate-800 flex items-center justify-center max-h-[380px]">
                      <img
                        src={imageSrc}
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 p-2.5 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 text-xs">
                      <div className="flex items-center gap-1">
                        <button
                          type="button"
                          onClick={() => setRotation((prev) => (prev + 90) % 360)}
                          className="px-2.5 py-1.5 bg-white dark:bg-slate-700 hover:bg-slate-100 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 font-semibold rounded-lg border border-slate-200 dark:border-slate-600 flex items-center gap-1.5 transition-colors cursor-pointer"
                        ...
                        <button
                          ...
                          className={`px-2.5 py-1.5 font-semibold rounded-lg border transition-colors cursor-pointer ${
                            grayscale ? 'bg-slate-800 dark:bg-slate-600 text-white border-slate-700' : 'bg-white dark:bg-slate-700 hover:bg-slate-100 text-slate-700 dark:text-slate-200 border-slate-200 dark:border-slate-600'
                          }`}
                        >
                          Grayscale
                        </button>
                        ...
                        <button
                          ...
                          className={`px-2.5 py-1.5 font-semibold rounded-lg border transition-colors cursor-pointer ${
                            highContrast ? 'bg-slate-800 dark:bg-slate-600 text-white border-slate-700' : 'bg-white dark:bg-slate-700 hover:bg-slate-100 text-slate-700 dark:text-slate-200 border-slate-200 dark:border-slate-600'
                          }`}
                        >
                          High Contrast
                        </button>
                        ...
                        <button
                          ...
                          className={`px-2.5 py-1.5 font-semibold rounded-lg border transition-colors cursor-pointer ${
                            inverted ? 'bg-slate-800 dark:bg-slate-600 text-white border-slate-700' : 'bg-white dark:bg-slate-700 hover:bg-slate-100 text-slate-700 dark:text-slate-200 border-slate-200 dark:border-slate-600'
                          }`}
                        >
                          Invert
                        </button>
                      ...
                      <button
                        ...
                        className="text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white text-[11px] font-medium cursor-pointer"
                      >
                        Reset Filters
                      </button>
                    </div>
          ...
          {/* OCR Progress Bar */}
          {isProcessing && (
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 shadow-xs border border-amber-200 dark:border-amber-800 animate-in fade-in">
        ...
        {/* Right Column: Editable Verified Result Form (6 cols) */}
        <div className="lg:col-span-6 space-y-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 shadow-xs border border-slate-200 dark:border-slate-800 transition-colors">
            {/* View Tabs */}
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-colors cursor-pointer ${
                    activeTab === 'extracted'
                      ? 'bg-amber-100 dark:bg-amber-950/60 text-amber-900 dark:text-amber-300 border border-amber-300/60 dark:border-amber-700'
            ...
            {/* Verification notice */}
            <div className="p-3 bg-amber-50/70 dark:bg-amber-950/30 border border-amber-200/80 dark:border-amber-800/60 rounded-xl text-amber-900 dark:text-amber-300 text-xs mb-4 flex items-start gap-2">
                  ...
                  <div className="space-y-3.5">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-xs font-semibold text-slate-900 dark:text-white focus:ring-2 focus:ring-amber-500"
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-xs font-semibold text-slate-900 dark:text-white focus:ring-2 focus:ring-amber-500 font-mono"
                        />
                      </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-xs font-medium text-slate-900 dark:text-white focus:ring-2 focus:ring-amber-500"
                      />
                    </div>
                    ...
                    <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex flex-wrap items-center justify-between gap-3">
                      <button
                        ...
                        className="text-xs font-semibold text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white py-2 px-3 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer"
                      >
                        Clear Fields
                      </button>
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2.5 px-5 rounded-xl text-xs shadow-xs transition-colors flex items-center gap-2 cursor-pointer"
                        >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-xs font-medium text-slate-900 dark:text-white focus:ring-2 focus:ring-amber-500 font-mono"
                        />
                      </div>
                ...
                <div className="relative">
                  ...
                  {rawOcrText && (
                    <button
                      ...
                      className="absolute top-3 right-3 bg-slate-800/80 hover:bg-slate-700 text-slate-300 p-1.5 rounded-md border border-slate-700 text-xs flex items-center gap-1 cursor-pointer"
                    >
