...
export const Navbar: React.FC<NavbarProps> = ({ onToggleSidebar, onNavigate, currentRoute }) => {
  ...
  return (
    <header className="bg-slate-900 text-white sticky top-0 z-40 shadow-md border-b border-slate-800">
      ...
      <div className="bg-gradient-to-r from-amber-700 via-amber-600 to-amber-700 text-amber-50 text-xs px-4 py-1.5 flex items-center justify-between font-medium tracking-wide">
        <div className="flex items-center gap-2">
          <span className="inline-block w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
          <span>ಸರ್ಕಾರದ ಸಾರ್ವಜನಿಕ ಗ್ರಂಥಾಲಯಗಳ ಇಲಾಖೆ | Department of Public Libraries, Govt. of Karnataka</span>
        </div>
        <div className="hidden md:flex items-center gap-4 text-[11px] opacity-95">
          <span className="flex items-center gap-1">
            <Clock className="w-3.5 h-3.5" /> {todayDate}
          </span>
          <span className="bg-amber-900/60 px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-wider">
            Official Portal
          </span>
        </div>
      </div>
      ...
      <div className="px-4 lg:px-6 py-2.5 flex items-center justify-between gap-4">
          ...
          >
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400 hidden sm:block">
                ಗಣಕೀಕೃತ ಗ್ರಂಥಾಲಯ ನಿರ್ವಹಣಾ ವ್ಯವಸ್ಥೆ | Gokak Central Unit
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 max-w-md hidden md:block">
          <button
            ...
            className="w-full bg-slate-800/80 hover:bg-slate-800 text-slate-400 hover:text-slate-200 border border-slate-700/80 hover:border-slate-600 rounded-xl px-4 py-2 flex items-center justify-between text-sm transition-colors shadow-inner"
          >
            <div className="flex items-center gap-2.5">
              <Search className="w-4 h-4 text-amber-400" />
              <span>Search books, members, accession no...</span>
            </div>
        ...
        {/* Right: Quick OCR, Role & User profile */}
        <div className="flex items-center gap-2.5">
          <button
            id="nav-quick-ocr-btn"
            onClick={() => onNavigate('ocr-scanner')}
            className="flex items-center gap-1.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold px-3 py-1.5 rounded-lg text-xs transition-all shadow-sm active:scale-95"
          ...
          {user ? (
            <div className="flex items-center gap-2 pl-2 border-l border-slate-700">
              <div className="hidden sm:flex flex-col text-right">
                ...
                <span className="text-[10px] text-amber-400 font-medium">
                  {user.role === 'ADMIN' ? '👑 Chief Admin' : '📖 Librarian'}
                </span>
              </div>
            ...
            <button
              ...
              className="bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold px-3 py-1.5 rounded-lg border border-slate-700"
            >
              Sign In
            </button>
          )}
