...
export const TransactionHistoryView: React.FC<TransactionHistoryViewProps> = ({ onNavigate }) => {
  ...
  const handleExportCSV = () => {
    if (transactions.length === 0) {
      showToast('warning', 'No Records', 'No transactions found to export.');
      return;
    }
    exportToCSV(
      transactions,
      `circulation_transactions_${new Date().toISOString().slice(0, 10)}`,
      [
    ...
    showToast('info', 'Export Started', 'Circulation transactions exported to CSV.');
  };
  ...
  const handleExportPDF = () => {
    ...
    try {
      ...
      showToast('success', 'PDF Generated', 'Circulation audit report downloaded.');
    } catch (err: any) {
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 md:p-6 shadow-xs border border-slate-200 dark:border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4 transition-colors">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <button
            ...
            className="bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-bold px-3.5 py-2 rounded-xl text-xs border border-slate-300 dark:border-slate-700 flex items-center gap-1.5 transition-all cursor-pointer"
          >
          ...
          <button
            ...
            className="bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold px-4 py-2 rounded-xl text-xs shadow-xs flex items-center gap-1.5 transition-all cursor-pointer"
          >
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 shadow-xs border border-slate-200 dark:border-slate-800 space-y-3 transition-colors">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 border-t border-slate-100 dark:border-slate-800 text-xs">
          <div className="flex items-center gap-2">
            <span className="text-slate-500 dark:text-slate-400 font-medium">From Date:</span>
            <input
              type="date"
              value={fromDate}
              onChange={(e) => { setFromDate(e.target.value); setPage(1); }}
              className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-2.5 py-1 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-amber-500"
            />
          </div>
          <div className="flex items-center gap-2">
            <span className="text-slate-500 dark:text-slate-400 font-medium">To Date:</span>
            <input
              type="date"
              value={toDate}
          ...
          <table className="w-full text-left border-collapse text-xs">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-slate-700 dark:text-slate-300">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={8} className="py-12 text-center text-slate-400 dark:text-slate-500">
                    <p className="font-semibold text-slate-600 dark:text-slate-300">No transactions found matching your criteria.</p>
                  </td>
                </tr>
              ) : (
                transactions.map((tx) => (
                  <tr key={tx.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="py-3 px-4 font-semibold text-slate-800 dark:text-slate-200">{tx.due_date}</td>
                    ...
                    <td className="py-3 px-4 font-mono font-bold">
                      {tx.fine_amount ? <span className="text-rose-600 dark:text-rose-400">₹{tx.fine_amount}</span> : '₹0'}
                    </td>
                    <td className="py-3 px-4 text-[11px] text-slate-500 dark:text-slate-400">{tx.issued_by_name || 'Librarian'}</td>
                  </tr>
                ))
              )}
            </tbody>
