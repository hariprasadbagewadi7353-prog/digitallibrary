    ...
    <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-2xl p-5 shadow-xs border border-slate-200 space-y-4">
          ...
          {!selectedMember ? (
            <div className="space-y-3">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs font-medium text-slate-900 placeholder-slate-400 focus:ring-2 focus:ring-amber-500 focus:bg-white"
                />
              </div>
                ...
                <div className="border border-slate-200 rounded-xl divide-y divide-slate-100 max-h-48 overflow-y-auto bg-white shadow-md">
                  {memberResults.map((m) => (
                    <button
                      ...
                      className="w-full p-3 text-left hover:bg-amber-50 flex items-center justify-between text-xs transition-colors"
                    >
                      <div>
                        <p className="font-bold text-slate-900">{m.student_name}</p>
                        <p className="text-[11px] font-mono text-slate-500">{m.registration_number} • {m.village}</p>
                      </div>
              ...
              {memberQuery && memberResults.length === 0 && (
                <p className="text-xs text-slate-500 text-center py-3">No members found with that search term.</p>
              )}
            </div>
          ) : (
            ...
            <div className="p-4 bg-amber-50/70 border border-amber-200 rounded-xl space-y-2">
              ...
              <h3 className="font-bold text-slate-900 text-sm">{selectedMember.student_name}</h3>
              <p className="text-xs text-slate-600">Village: {selectedMember.village} | Mobile: {selectedMember.mobile}</p>

              <div className="pt-2 border-t border-amber-200 flex items-center justify-between text-xs">
                <span>Active Loans:</span>
                <span className="font-bold text-slate-900">{memberActiveLoanCount} / 3 Books</span>
              </div>
            </div>
          )}
        </div>
          ...
          {!selectedBook ? (
            <div className="space-y-3">
                  ...
                  {bookResults.map((b) => (
                    ...
                    >
                      <div className="min-w-0 flex-1 pr-2">
                        ...
                        <p className="text-[11px] font-mono text-slate-500">{b.book_number} • By {b.author}</p>
                      </div>
              ...
              {bookQuery && bookResults.length === 0 && (
                <p className="text-xs text-slate-500 text-center py-3">No available books matched your search.</p>
              )}
            </div>
          ) : (
            /* Selected Book Card */
            <div className="p-4 bg-emerald-50/70 border border-emerald-200 rounded-xl space-y-2">
              ...
              <h3 className="font-bold text-slate-900 text-sm line-clamp-1">{selectedBook.title}</h3>
              <p className="text-xs text-slate-600">By {selectedBook.author} | Shelf: {selectedBook.shelf_location || 'Rack A-01'}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl p-6 shadow-xs border border-slate-200 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs font-semibold text-slate-900 focus:ring-2 focus:ring-amber-500"
            />
          </div>
        ...
        <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 text-xs text-slate-600 flex items-start gap-3">
          ...
          <div>
            <p className="font-bold text-slate-800">Circulation Policy Rules:</p>
            <p className="mt-0.5">
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          <button
            ...
            className="px-5 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-100 rounded-xl border border-slate-300"
          >
            Cancel
          </button>
          ...
          <button
            ...
            className="bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white font-bold py-2.5 px-6 rounded-xl text-xs shadow-xs transition-colors flex items-center gap-2"
          >
