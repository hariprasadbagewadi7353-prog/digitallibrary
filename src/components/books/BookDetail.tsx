  ...
  if (isLoading) {
    return (
      <div className="bg-white rounded-2xl p-12 text-center text-slate-500 border border-slate-200">
        ...
        <p className="font-semibold text-slate-700">Loading catalog record & circulation status...</p>
      </div>
    );
  }
  ...
  if (!book) {
    return (
      <div className="bg-white rounded-2xl p-8 text-center border border-slate-200">
        <p className="text-slate-600 font-bold">Book catalog entry not found.</p>
        <button
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5">
          <button
            ...
            className="bg-slate-900 hover:bg-slate-800 text-amber-300 font-bold px-4 py-2 rounded-xl text-xs flex items-center gap-1.5 shadow-2xs transition-colors border border-slate-700"
          >
      ...
      <div className="bg-white rounded-2xl p-6 shadow-xs border border-slate-200">
        <div className="flex flex-col md:flex-row items-start gap-6 pb-6 border-b border-slate-100">
          {/* Book Spine / Visual */}
          <div className="w-28 h-40 rounded-xl bg-slate-900 text-amber-400 flex flex-col items-center justify-center p-3 text-center shrink-0 border border-slate-800 shadow-md">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-2">
              ...
              <span className="text-xs text-amber-800 font-semibold bg-amber-50 border border-amber-200/60 px-2 py-0.5 rounded">
            ...
            {/* Synopsis */}
            <p className="text-xs text-slate-600 mt-3 leading-relaxed max-w-3xl bg-slate-50 p-3 rounded-xl border border-slate-200">
              {book.description || 'This book is an official acquisition under the Karnataka Public Libraries Act, maintained for public reference, research, and academic loans.'}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-6 text-xs">
          <div>
            ...
            <span className="font-semibold text-slate-800">{book.publisher || 'Not Specified'}</span>
          </div>

          <div>
            <span className="text-slate-400 block text-[10px] uppercase font-bold">Publication Year</span>
            <span className="font-semibold font-mono text-slate-800">{book.publication_year || 'N/A'}</span>
          </div>

          <div>
            <span className="text-slate-400 block text-[10px] uppercase font-bold">ISBN</span>
            <span className="font-semibold font-mono text-slate-800">{book.isbn || 'N/A'}</span>
          </div>

          <div>
            <span className="text-slate-400 block text-[10px] uppercase font-bold">Shelf / Rack</span>
            <span className="font-semibold font-mono text-slate-800">{book.shelf_location || 'Rack A-01'}</span>
          </div>
        </div>
      </div>
      ...
      {currentActiveLoan && (
        <div className="bg-amber-50/80 border border-amber-300/80 rounded-2xl p-5 shadow-xs">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 text-xs text-slate-600 mt-1">
                  ...
                  <span>Due Date: <strong className={currentActiveLoan.status === 'OVERDUE' ? 'text-rose-600' : ''}>{currentActiveLoan.due_date}</strong></span>
                </div>
              </div>
            </div>
            ...
            <button
              ...
              className="bg-blue-600 hover:bg-blue-700 text-white font-bold px-4 py-2 rounded-xl text-xs shadow-xs transition-colors flex items-center gap-1.5"
            >
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700">
                {transactions.map((tx) => (
                  <tr key={tx.id} className="hover:bg-slate-50/80">
                    <td className="py-3 px-4 font-bold text-slate-900">{tx.member_name}</td>
